...
object AOC10
{
	...
	val moves = Map('-'->(L,L), '|'->(U,U), 'F'->(U,R), '7'->(U,L), 'J'->(D,L), 'L'->(D,R))

	case class Move(dx: Int, dy: Int) {
		def rev = Move(-dx, -dy)
	}

	type PipeFn = (Move) => Option[Move]
	case class Pipe(c: Char, fn: PipeFn) {
		def update(c2: Char) = Pipe(c2, fn)
		override def toString: String = ""+c
	}

	val noPipe: PipeFn = (move) => None
	def pipe(p: (Move,Move)):PipeFn = (move) => {
		if (p._1==move) Some(p._2)
		else if (p._2.rev==move) Some(p._1.rev)
		else None
	}
	def makePipe(c: Char): Pipe = {
		Pipe(c, moves.get(c).map(pipe).getOrElse(noPipe))
	}

	def extract(lines: List[String]): Pipes = {
		val starts = lines.map(_.indexOf('S'))
		val start = Point(starts.find(_>=0).get, starts.indexWhere(_>=0))
		println(s"W = ${lines.head.size} H = ${lines.size} Start = $start")
		Pipes(lines.toArray.map(extract), start)
	}
	def extract(line: String): Array[Pipe] = line.toArray.map(makePipe)

	case class Point(x: Int, y: Int) {
		def next(m: Move) = Point(x+m.dx, y+m.dy)
		def delta(p2: Point) = Move(p2.x-x, p2.y-y)
	}

	/** A grid of "pipes", where a pipe is a function that will take a direction of movement, e.g. (0,1),
	 *  and either turn it into another direction (if it's a valid pipe that points in the original direction),
	 *  or return None. */
	case class Pipes(a: Array[Array[Pipe]], start: Point) {
		val w = a(0).size
		val h = a.size

		def print() = a.foreach(line => println(line.mkString("")))

		def count(c: Char) = a.map(_.count(_.c==c)).sum

		def get(p: Point): Option[Pipe] = if (p.x>=0 && p.x<w && p.y>=0 && p.y<h) Some(a(p.y)(p.x)) else None

		def update(p: Point, c: Char) {
			a(p.y)(p.x) = a(p.y)(p.x).update(c)
		}

		def findCycles: List[List[Point]] = {
			List(L,R,U,D).map(m=> follow(start, m, Nil)).flatten
		}

		def follow(p: Point, d: Move, pathSoFar: List[Point]): Option[List[Point]] = {
			val nextP = p.next(d)
			if (pathSoFar.contains(nextP)) {
				if (nextP==pathSoFar.last) Some(p::pathSoFar)
				else None
			}
			else {
				val pipe = get(nextP)
				if (pipe.nonEmpty)
				{
					val nextMove = pipe.get.fn(d)
					if (nextMove.nonEmpty)
						follow(nextP, nextMove.get, p::pathSoFar)
					else
						None
				} else
					None
		...
		/** Implement odd-even algorithm on the x axis. It will replace - characters with I where the square is
		 *  "inside" the pipe. You must have called {@link leaveOnlyCycle()} first! */
		def markInners(cycle: List[Point]) {
			a.foreach(inner => {
				var inside = false
				// Last is used to track non-U shapes which act as a single line: FJ or F--J etc, L7 or L--7 etc
				// Unlike U shapes (F--7), these change the parity after they've finished.
				var last = ""
				for (i <- 0 until inner.size) {
					...
					if (c!='.')
					{
						if (c!='-') {
							if (last.isEmpty) {
								inside = !inside
								if ("FL".contains(c)) last = ""+c
							}
							else {
								if (!Set("FJ","L7").contains(last+c)) inside = !inside
								last = ""
							}
						}
					}
					else if (inside) {
						inner(i) = inner(i).update('I')
					}
				}
			})
		}

		def forALl(apply: (Point=>Unit)): Unit = {
			(0 until w).foreach(x => (0 until h).foreach(y=> apply(Point(x,y)) ))
		}

		/** Leaves the pipe characters in place for the given cycle, but resets everything else to '.'.
		 *  Also changes the "S" character to the actual correct pipe symbol. */
		def leaveOnlyCycle(cycle: List[Point]) {
			forALl(p => {if (!cycle.contains(p)) update(p, '.')})
			// Replace the S!
			val p1 = cycle.head
			val p2 = cycle.reverse.head	// This is the starting point; get(p2) = 'S'
			val p3 = cycle.reverse.tail.head
			val d1 = p1.delta(p2)
			val d2 = p2.delta(p3)
			val ds = Set( (d1,d2), (d2.rev,d1.rev))
			val c = moves.find(p => ds(p._2)).get._1
			update(p2, c)
		}
