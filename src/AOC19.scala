...
object AOC19
{
	...
	case class Workflow(name: String, rules: List[Rule]) {
		def process(part: Part): String = {
			var rem = rules
			while (rem.nonEmpty) {
				val a = rem.head.accept(part)
				if (a.nonEmpty) return a.get
				rem = rem.tail
			}
			throw new Exception(s"Workflow $name didn't terminate")
		}
		def process(partRanges: List[PartRange]): List[(PartRange,String)] = {
			var matched = List[(PartRange,String)]()
			var rem = rules
			var processing = partRanges
			while (rem.nonEmpty) {
				var unmatched = List[PartRange]()
				for (pr <- processing) {
					val (u, m) = rem.head.accept(pr)
					if (m.nonEmpty) matched = (m.get)::matched
	...
	case class WorkflowSet(workflows: Map[String,Workflow]) {
		def process(part: Part): Long = {
			var action = workflows("in").process(part)
			while (action!="A" && action!="R") {
				action = workflows(action).process(part)
			}
			if (action=="A") part.rating.toLong else 0L
		}
		def processRanges: Long = {
			var results = List[PartRange]()
			val tr = (1,4000)
			val tpr = PartRange(Map("x"->tr,"m"->tr,"a"->tr,"s"->tr))
			var actions = workflows("in").process(List(tpr))
			results = results++actions.filter(_._2=="A").map(_._1)
			var toProcess = actions.filter(p => p._2!="A" && p._2!="R")
			while (toProcess.nonEmpty) {
				var nextActions = List[(PartRange,String)]()
				toProcess.foreach(p => {
					actions = workflows(p._2).process(List(p._1))
					results = results++actions.filter(_._2=="A").map(_._1)
					nextActions = nextActions ++ actions.filter(p => p._2!="A" && p._2!="R")
				})
				toProcess = nextActions
			}
			results.map(_.rating).sum
		}
	}
